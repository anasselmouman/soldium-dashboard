# -*- coding: utf-8 -*-
"""Phase 9L — expansion validation tests (fixtures + artifact checks)."""
...
def _ready_publish(conn, *, name="خدمة", external="111"):
    ...
    result = pub.publish(svc.id, published_by="phase9l_test")
...
def test_execution_source_history_no_auto_republish(catalog_db: Path):
    with catalog_transaction(catalog_db) as conn:
        ...
        assert any(
            h.status == "historical" and h.external_service_id == "OLD-ID" for h in hist
        )
...
def test_artifact_if_present():
    path = Path("scripts/out_phase9l_expansion_validation_2.json")
    if not path.exists():
        pytest.skip("9L artifact missing")
    data = json.loads(path.read_text(encoding="utf-8"))
    assert data["verdict"] in {
        "PHASE_9L_COMPLETE — EXPANSION_VALIDATED",
        "PHASE_9L_BLOCKED — REVIEW_REQUIRED",
    }
    ...
    assert data["production_baseline"]["publications"] == 53
    ...
    assert data["shadow"]["aggregate"]["catalog_count"] == 43
    assert data["drift_analysis"]["drifted_count"] == 0
    assert data["order_intent_safety"]["unchanged"] is True
    assert data["EXPANSION_DECISION"] in {
        "EXPANSION_READY",
    ...
    assert data["next_step"] in {
        ...
        "PAUSE_AND_REMEDIATE",
        "BEGIN_CUTOVER_PREPARATION",
    }
    cohorts = data["three_cohorts"]
    assert cohorts["original13"]["count"] == 13
    assert cohorts["wave1"]["count"] == 15
    assert cohorts["wave2"]["count"] == 15
